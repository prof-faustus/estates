...
|---|---------|-----|--------------|
| 1 | Actions unauthenticated / forgeable | `sidecar` GamePeer SIGNS every move with the seat's identity key over `(table, turn, actor, action)`; a peer may only move its own seat; a move is applied only if its signature verifies against the active seat's registered key. | `packages/sidecar/test` — "a badly-SIGNED move is rejected" (real sockets) |
