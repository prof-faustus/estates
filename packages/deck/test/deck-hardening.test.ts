...
test('dealerlessDeckOrder is jointly generated by ALL parties (no single party determines it)', async () => {
  const { dealerlessDeckOrder } = await import('../src/index.ts');
  const mk = (seat: number, n: number): SeedParty => {
    const reveal = new Uint8Array(32).fill(n);
    return { seat, pub: bytesToHex(genCardKey().pub), commitment: commitEntropy(reveal), reveal };
  };
  const TABLE2 = 'a1'.repeat(32);
  const parties = [mk(0, 1), mk(1, 2), mk(2, 3)];
  const sizes = { Fate: 12, Treasury: 12 };
  const order = dealerlessDeckOrder(parties, TABLE2, sizes)!;
  // valid permutations for each deck
  for (const name of Object.keys(sizes)) {
    assert.deepEqual([...order[name]!].sort((a, b) => a - b), Array.from({ length: 12 }, (_, i) => i), `${name} is an exact permutation`);
  }
  // deterministic + order-independent (combineSeedBound sorts by seat)
  assert.deepEqual(dealerlessDeckOrder([parties[2]!, parties[0]!, parties[1]!], TABLE2, sizes), order);
  // changing ANY party's entropy changes the order (no single party fixed it, none ignored)
  const changed = [mk(0, 9), parties[1]!, parties[2]!];
  assert.notDeepEqual(dealerlessDeckOrder(changed, TABLE2, sizes)!.Fate, order.Fate);
  // a party whose reveal does not open its commitment → null (rejected)
  const bad: SeedParty = { ...parties[0]!, commitment: commitEntropy(new Uint8Array(32).fill(99)) };
  assert.equal(dealerlessDeckOrder([bad, parties[1]!, parties[2]!], TABLE2, sizes), null);
  // bad gameId → null
  assert.equal(dealerlessDeckOrder(parties, 'zz'.repeat(32), sizes), null);
});
